...
void top_level_task(const Task *task,
	...
	//
	IndexTaskLauncher task2_launcher(READING_TASK_ID, is, TaskArgument(&num_pieces,sizeof(int)), arg_map);
	...
	runtime->execute_index_space(ctx, task2_launcher);
...
void init_reg(const Task *task,
	...
	if (index==0 || index==num_pieces-1) acc_reg1[point] = 1;
	else acc_reg1[point] = 0;
	acc_reg2[point] = index;
...
void reg2_task(const Task *task,
	...
	const FieldAccessor<READ_ONLY, double, 1> acc_reg1(regions[0], REG1_ID);
	const FieldAccessor<WRITE_ONLY, double, 1> acc_reg2(regions[1], REG2_ID);
	...
	if (index==0 || index==num_pieces-1){
		if (index==0) acc_reg2[index] = acc_reg1[num_pieces-1];
		if (index==num_pieces-1) acc_reg2[index-1] = acc_reg1[index-1];
	}
	else acc_reg2[index] = acc_reg1[index-1];
